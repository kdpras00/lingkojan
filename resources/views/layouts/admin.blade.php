...
<head>
    ...
    <title>@yield('title') - LingKojan Admin</title>
...
<body style="margin: 0; min-height: 100vh; background: transparent;">
    <nav class="navbar-main relative">
        <div class="navbar-container">
            ...
            <div class="hidden md:flex items-center space-x-6">
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('admin.menu') }}" class="nav-link {{ Request::routeIs('admin.menu') ? 'active' : '' }}">Menu</a>
                </div>
                ...
                <div class="user-dropdown">
                    <div class="user-trigger">
                        ...
                        <span class="text-base font-semibold text-gray-700">{{ Auth::user()->name ?? 'Administrator' }}</span>
                    </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden absolute top-full left-0 right-0 w-full bg-white shadow-lg border-b border-gray-200 z-50 transition-all duration-300 ease-out transform -translate-y-2 opacity-0">
            <div class="px-6 py-5 space-y-3 flex flex-col">
                <a href="{{ route('admin.dashboard') }}" class="w-full py-2.5 px-4 rounded-xl nav-link {{ Request::routeIs('admin.dashboard') ? 'active bg-orange-50/50' : 'text-gray-700' }} hover:bg-orange-50 hover:text-[#f07c1b] transition-all duration-200">Dashboard</a>
                <a href="{{ route('admin.menu') }}" class="w-full py-2.5 px-4 rounded-xl nav-link {{ Request::routeIs('admin.menu') ? 'active bg-orange-50/50' : 'text-gray-700' }} hover:bg-orange-50 hover:text-[#f07c1b] transition-all duration-200">Menu</a>
    ...
    <main class="main-content">
        <div class="mb-10 text-center md:text-left">
            <h2 class="text-3xl font-bold text-black">@yield('page_title', 'Dashboard Admin')</h2>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // NIK: only digits allowed, max 16
        document.querySelectorAll('input[name="nik"]').forEach(function(el) {
            ...
            el.addEventListener('paste', function(e) {
                ...
                this.value = (this.value + pasted).replace(/[^0-9]/g, '').slice(0, 16);
            });
        });

        // Phone: only digits allowed, max 15
        document.querySelectorAll('input[name="phone"]').forEach(function(el) {
            ...
            el.addEventListener('paste', function(e) {
                e.preventDefault();
                var pasted = (e.clipboardData || window.clipboardData).getData('text');
                this.value = (this.value + pasted).replace(/[^0-9]/g, '').slice(0, 15);
            });
        });

        // Username: only alphanumeric allowed, no space/symbol
        document.querySelectorAll('input[name="username"]').forEach(function(el) {
            ...
            el.addEventListener('paste', function(e) {
                e.preventDefault();
                var pasted = (e.clipboardData || window.clipboardData).getData('text');
                this.value = (this.value + pasted).replace(/[^a-zA-Z0-9]/g, '').slice(0, 50);
            });
        });
