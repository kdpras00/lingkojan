...
<div class="space-y-8">
    ...
    <div class="card !p-10 space-y-10">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100 pb-8">
            ...
            <a href="{{ route('admin.petugas.create') }}" class="bg-black text-white px-8 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-gray-800 transition-all shadow-lg flex items-center group">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-5 text-sm font-semibold text-gray-700 border-r border-gray-100 text-center">{{ $item->rt->nama_rt ?? '-' }}</td>
                        <td class="px-6 py-5">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <a href="{{ route('admin.petugas.edit', $item->id) }}" class="p-1 text-gray-400 hover:text-orange-600 transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <button type="button" onclick="openDeleteModal('{{ $item->nama_warga }}', {{ $item->id }})" class="p-1 text-gray-400 hover:text-red-600 transition-colors" title="Hapus">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm">
        <div class="bg-white rounded-[32px] overflow-hidden shadow-2xl border border-gray-100">
            <div class="p-10 text-center space-y-6">
                <div>
                    <h4 class="text-xl font-black text-black tracking-tight" id="modal_title">Hapus User</h4>
...
<script>
    function openDeleteModal(name, id) {
        ...
        document.getElementById('delete_form').action = '/admin/petugas/' + id;
