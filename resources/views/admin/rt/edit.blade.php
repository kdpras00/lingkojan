...
@section('page_title', 'Edit Data RT')
...
<div class="space-y-8">
    ...
    <div class="flex justify-center py-10">
        <div class="card w-full max-w-xl !p-12 shadow-2xl">
            <form action="{{ route('admin.rt.update', $rt->id) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                <div class="flex items-center space-x-10">
                    <label class="text-sm font-black text-black uppercase tracking-widest w-40">Nomor RT</label>
                    <div class="flex-1 group">
                        <input type="text" name="nomor" value="{{ old('nomor', $rt->nomor) }}" class="w-full bg-gray-50 border @error('nomor') border-red-500 @else border-gray-200 @enderror rounded-2xl px-6 py-4 text-sm font-bold text-gray-700 focus:outline-none focus:ring-4 focus:ring-orange-500/5 focus:border-[#f07c1b] transition-all" placeholder="Contoh: 001">
                ...
                <div class="flex items-center space-x-10">
                    ...
                    <div class="flex-1 group">
                        <input type="text" name="nama_ketua" value="{{ old('nama_ketua', $rt->nama_ketua) }}" class="w-full bg-gray-50 border @error('nama_ketua') border-red-500 @else border-gray-200 @enderror rounded-2xl px-6 py-4 text-sm font-bold text-gray-700 focus:outline-none focus:ring-4 focus:ring-orange-500/5 focus:border-[#f07c1b] transition-all" placeholder="Nama Ketua RT">
