...
<div class="space-y-8">
    ...
    <div class="mb-10 flex items-center text-sm font-bold text-black px-4 md:px-0">
        ...
        <a href="{{ route('admin.ketua_rt.index') }}" class="hover:text-[#f07c1b]">Data Ketua RT</a>
        <span class="mx-3 text-gray-300 font-normal text-lg">/</span>
        <span class="text-gray-400">Tambah</span>
    </div>

    <div class="card !p-12">
        <form action="{{ route('admin.ketua_rt.store') }}" method="POST" class="space-y-12">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    ...
                    <div class="group">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" class="w-full bg-gray-50 border @error('email') border-red-500 @else border-gray-200 @enderror rounded-2xl px-6 py-4 text-sm font-bold text-gray-700 focus:outline-none focus:ring-4 focus:ring-orange-500/5 focus:border-[#f07c1b] transition-all" placeholder="Contoh: ketuart@gmail.com">
